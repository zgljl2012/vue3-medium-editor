...
export default class Core {
  ...
  initAddons () {
    ...
    this._plugin._initializedAddons = {
      images: new Images(
        this._plugin,
        this._plugin.addons.images,
        this.MediumEditor
      ),
      embeds: new Embeds(
        this._plugin,
        this._plugin.addons.embeds,
        this.MediumEditor
      )
    }
    ...
    Object.keys(this._plugin.addons).forEach(name => {
      ...
      if (!this._plugin._initializedAddons[name]) {
        if (typeof AddonOptions === 'function') {
          this._plugin._initializedAddons[name] = new AddonOptions(
            this._plugin
          )
        } else {
          window.console.error(
            `I don't know how to initialize custom '${name}' addon!`
          )
        }
      }
